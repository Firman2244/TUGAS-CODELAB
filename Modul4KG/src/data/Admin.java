...
public class Admin extends User implements iMenu{
    ...
    public void login() {
        ...
        System.out.print("input Password (admin): ");
        String password = scanner.next();
        if (isAdmin(username, password)) {
            System.out.println("Login Success sebagai Admin");
            menu();
        } else {
    ...
    public void addStudent() {
        ...
        addTempStudent.add(new Student(nim, name, faculty, studyProgram));
    ...
    private void inputBook() {
        ...
        String bookId, judul, author, category;
        int stok;
        ...
        switch (bookType) {
            case 1:
                bookId = generateId("BH");
                addTempBooks[i++] = new HistoryBook(bookId, judul, stok, category, author);
                break;
            case 2:
                bookId = generateId("BS");
                addTempBooks[i++] = new StoryBook(bookId, judul, stok, category, author);
                break;
            case 3:
                bookId = generateId("BT");
                addTempBooks[i++] = new TextBook(bookId, judul, stok, category, author);
                break;
            default:
                System.out.println("Pilihan tidak valid!");
                return;
    ...
    private String generateId(String prefix) {
        int nextId = i + 1;
        return prefix + String.format("%ddr", nextId);
    }
}
